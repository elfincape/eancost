...
현재 코드는 단계형 개발 계획 중 **4단계: 정산자료 변환/붙여넣기용 결과 생성**까지 구현되어 있습니다. `/upload` 화면에서 .xlsx 파일을 선택하면 SheetJS 기반 파서가 브라우저에서 시트 목록과 미리보기 테이블을 표시하고, 파싱 결과를 localStorage에 임시 저장합니다. `/settings/vehicles`에서 고정차량 목록을 등록하면 `/settlements`에서 전체/고정차/임시차 탭으로 분류 결과를 확인할 수 있습니다. `/settlements/convert`에서는 원본 엑셀을 다시 업로드하거나 마지막 업로드 데이터를 사용해 고정차만 추출하고, 표준 컬럼 매핑 후 붙여넣기용 테이블/클립보드 복사/xlsx 다운로드를 제공합니다. 아직 DB 저장, 복잡한 정산 계산, 청구/지급 비교 검증은 수행하지 않습니다.
...
1. **5단계: 청구/지급 비교 검증**
   - 청구 파일과 지급 파일의 차량번호 기준 비교
   - 누락, 중복, 금액 차이 탐지 및 다운로드
2. **6~7단계: BI 대시보드와 위클리 리포트**
   - 업로드/파싱 데이터 기반 센터별 현황, 차트, 주차별 보고 문장 생성
3. **8~9단계: 저장 구조와 권한**
