...
현재 코드는 단계형 개발 계획 중 **2단계: 엑셀 업로드 및 파싱**까지 구현되어 있습니다. `/upload` 화면에서 .xlsx 파일을 선택하면 SheetJS 기반 파서가 브라우저에서 시트 목록과 미리보기 테이블을 표시합니다. 아직 DB 저장, 고정차 필터링, 정산 계산은 수행하지 않습니다.
...
1. **3단계: 고정차/임시차 구분**
   - localStorage 기반 고정차량 목록 등록/삭제
   - 차량번호 기준 매칭 함수 분리
   - `/settlements` 전체/고정차/임시차 필터 탭 연결
2. **4단계: 정산자료 변환/붙여넣기 결과 생성**
