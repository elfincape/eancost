...
현재 코드는 단계형 개발 계획 중 **3단계: 고정차/임시차 구분**까지 구현되어 있습니다. `/upload` 화면에서 .xlsx 파일을 선택하면 SheetJS 기반 파서가 브라우저에서 시트 목록과 미리보기 테이블을 표시하고, 파싱 결과를 localStorage에 임시 저장합니다. `/settings/vehicles`에서 고정차량 목록을 등록하면 `/settlements`에서 전체/고정차/임시차 탭으로 분류 결과를 확인할 수 있습니다. 아직 DB 저장, 정산 변환, 청구/지급 비교 계산은 수행하지 않습니다.
...
1. **4단계: 정산자료 변환/붙여넣기 결과 생성**
   - `/settlements/convert` 페이지 추가
   - 원본 컬럼 → 표준 컬럼 매핑 UI
   - 고정차만 추출 후 복사/xlsx 다운로드 지원
2. **5단계: 청구/지급 비교 검증**
   - 청구 파일과 지급 파일의 차량번호 기준 비교
   - 누락, 중복, 금액 차이 탐지 및 다운로드
3. **6~7단계: BI 대시보드와 위클리 리포트**
